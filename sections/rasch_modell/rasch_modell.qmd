...
Die Funktion bildet den Wert des linearen Prädiktors (reelle Zahl) auf eine Wahrscheinlichkeit, also einen Wert im Intervall $[0, 1]$ ab. Es entsteht eine logistische Regression mit Personen- und Itemparametern als Prädiktoren.
...
\begin{equation}
	P(U_1 = u_1, ..., U_m = u_m | \theta_i) = \prod_{j = 1}^{m} P(U_j = u_j | \theta_i)
\end{equation}

::: {.callout-aufgabe}

Auf Basis der diagnostischen Rechenkompetenzen und der Sitzung zu Datenmatrizen haben Sie ausreichen Vorwissen, um die Formel zur lokalen stochastischen Unabhängigkeit vollständig nachzuvollziehen. Verbalisieren Sie die Formel von links nach rechts und Symbol für Symbol.
...
Bereits intuitiv muss die Aussage falsch sein. Wenn man mit $m$ Items tatsächlich ein gemeinsames Konstrukt misst, werden Itemantworten (also die Spaltenvektoren) miteinander korrelieren. Stellen Sie sich zum Beispiel vor, es würde sich um $m = 2$ Offenheits-Items handeln. Sicherlich werden die Antworten auf zwei Items "Ich mag Gedichte" und "Ich glaube, dass Kunst wichtig ist." sehr hoch miteinander korrelieren. Sie messen ja ein gemeinsames Konstrukt. Die Rohdaten sind also, entgegen der Behauptung, mit hoher Wahrscheinlichkeit paarweise voneinander abhängig.
...
Installieren Sie das Paket eRm in R und entnehmen Sie der help-File des Befehls RM(), wie die latente Skala im Paket identifiziert wird.
...
Was ist der Mittelwert der Itemparameter?
...
Es wurden 500 Testand:innen vermessen. Warum hat die Tabelle der Personenparameter nur 16 Zeilen?
